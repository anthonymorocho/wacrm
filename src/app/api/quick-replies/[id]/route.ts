...
import { validateQuickReplyMedia } from '@/lib/inbox/quick-reply-media';
...
export async function PATCH(
  ...
  const hasMediaInput =
    'media_url' in body || 'media_path' in body || 'media_filename' in body;
  if (hasMediaInput) {
    const media = validateQuickReplyMedia(body, ctx.accountId);
    if (!media.ok) {
      return NextResponse.json({ error: media.error }, { status: 400 });
    }
    update.media_url = media.media_url;
    update.media_path = media.media_path;
    update.media_filename = media.media_filename;
  }
  ...
  if ('kind' in body) {
    ...
    if (body.kind === 'interactive') {
      if (hasMediaInput || update.media_url) {
        return NextResponse.json(
          { error: 'Images are only supported on text quick replies' },
          { status: 400 }
        );
      }
