...
function previewFor(step: BuilderStep, t: ReturnType<typeof useTranslations>): string {
  switch (step.step_type) {
    ...
    case "send_buttons":
    case "send_list":
      return interactivePayloadPreviewText(asInteractive(step.step_config)) || t("previews.noBody")
    case "send_template":
      return (step.step_config.template_name as string) || t("previews.selectTemplate")
    case "wait": {
      const unit = step.step_config.unit
      const amount = step.step_config.amount ?? "?"
      const singular = amount === 1 || amount === "1"
      const unitKey = unit === "minutes"
        ? singular ? "previews.units.minute" : "previews.units.minutes"
        : unit === "days"
        ? singular ? "previews.units.day" : "previews.units.days"
        : singular ? "previews.units.hour" : "previews.units.hours"
      return `${String(amount)} ${t(unitKey)}`
    }
    case "condition": {
      const subject = step.step_config.subject
      const subjectKey = subject === "contact_field"
        ? "config.subjects.contact_field"
        : subject === "message_content"
        ? "config.subjects.message_content"
        : subject === "time_of_day"
        ? "config.subjects.time_of_day"
        : "config.subjects.tag_presence"
      return t("previews.when", { subject: t(subjectKey) })
    }
    case "send_webhook":
