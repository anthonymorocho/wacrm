...
describe('conversation visibility', () => {
  it('lets owners and admins see assigned and queued conversations', () => {
    ...
    expect(isConversationVisibleToUser(assigned, 'owner', 'owner-1')).toBe(
      true
    );
    expect(isConversationVisibleToUser(queued, 'admin', 'admin-1')).toBe(true);
    expect(
      filterVisibleConversations([assigned, queued], 'admin', 'admin-1')
    ).toHaveLength(2);
  });
  ...
  it('lets an agent see their assignments and the shared queue', () => {
    ...
    expect(
      filterVisibleConversations([mine, theirs, queued], 'agent', 'agent-1')
    ).toEqual([mine, queued]);
  });
  ...
  it('lets viewer roles see queued conversations but not assigned conversations', () => {
    ...
    expect(isConversationVisibleToUser(queued, 'viewer', 'viewer-1')).toBe(
      true
    );
    expect(
      isConversationVisibleToUser(closedQueued, 'viewer', 'viewer-1')
    ).toBe(false);
    expect(isConversationVisibleToUser(closedQueued, 'admin', 'admin-1')).toBe(
      true
    );
    expect(
      filterVisibleConversations([mine, queued], 'viewer', 'viewer-1')
    ).toEqual([queued]);
    expect(
      filterVisibleConversations([queued, closedQueued], 'viewer', 'viewer-1')
    ).toEqual([queued]);
  });
});

describe('conversation assignment kind', () => {
  it('treats a closed transferred conversation as owned for the next cycle', () => {
    expect(
      getConversationAssignmentKind({ status: 'open', was_transferred: false })
    ).toBe('owned');
    expect(
      getConversationAssignmentKind({ status: 'open', was_transferred: true })
    ).toBe('transferred');
    expect(
      getConversationAssignmentKind({ status: 'closed', was_transferred: true })
    ).toBe('owned');
  });
});
